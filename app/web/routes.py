...
async def api_affiliate_get(_: bool = Depends(require_login)):
    from ..affiliate import AFFILIATE_KEYS, affiliate_ids

    ids = affiliate_ids(db.SessionLocal)
    return JSONResponse({k: ids.get(k, "") for k in AFFILIATE_KEYS})


@router.post("/api/affiliate")
async def api_affiliate_save(request: Request, body: dict, _: bool = Depends(require_login)):
    """Salva os IDs no banco — funciona igual na VPS e na Vercel."""
    from ..affiliate import AFFILIATE_KEYS, invalidate_cache
